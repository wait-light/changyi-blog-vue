<template>
    <el-row v-loading="loading" element-loading-text="拼命加载中">
        <el-col :xs="0" :sm="1" :md="4" :lg="4"><br /></el-col>
        <el-col :xs="24" :sm="20" :md="16" :lg="16">
            <div class="date_article_box basic_border_shadow">
                <el-collapse v-model="activeName" accordion class="basic_font_title_xs">
                    <el-collapse-item style="background: black;" v-for="(date,index) in dates"
                        :title='new Date().format_maxMouth(date)' :name='index + 1'>
                        <el-button @click='$toWhere("/artileDetai/" + article.id)'
                            style="width: 100%;font-size: medium;" v-for="article in articles[index]" size="small"
                            type="text">{{article.title}}</el-button>
                    </el-collapse-item>
                </el-collapse>
            </div>
        </el-col>
