<template>
    <div>
        <div class="friends_big_box" v-loading="loading" element-loading-text="拼命加载中">
            <el-row>
                <h2 class="basic_untouch" v-if="!friends || friends.length == 0">人生所贵在知己，四海相逢骨肉亲</h2>
                <el-col
                    :xs="24"
                    :md="6"
                    :lg="4"
                ...
                >
                    <div
                        class="friends_small_box basic_border_shadow friends_small_box_title"
                        @click="toWhere(friend.url)"
                    >
                        <div :src="friend.url">{{ friend.title }}</div>
